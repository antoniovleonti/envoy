...
LoopLatencyTracker::LoopLatencyTracker(const std::string& dispatcher_name,
                                       std::shared_ptr<LoopLatencyRegistry> registry)
    : dispatcher_name_(dispatcher_name) {
  if (registry != nullptr) {
    registration_handle_ = registry->registerTracker(*this);
  }
}
...
void LoopLatencyTracker::reportPrepare(uint64_t prepare_time_us, bool timeout_set,
  ...
  if (check_time_us_ != 0) {
    ...
    sum_combined_epoll_us_.fetch_add(combined, std::memory_order_relaxed);
  }
...
uint64_t LoopLatencyTracker::getSumCombinedEpollUs() const {
  return sum_combined_epoll_us_.load(std::memory_order_relaxed);
}
