...
public:
  ...
  virtual uint64_t getSumCombinedEpollUs() const = 0;
};

class LoopLatencyTracker : public LoopLatencyWriter, public LoopLatencyReader {
public:
  LoopLatencyTracker(const std::string& dispatcher_name, std::shared_ptr<LoopLatencyRegistry> registry);
  ~LoopLatencyTracker() override = default;

  // LoopLatencyReader
  const std::string& dispatcherName() const override { return dispatcher_name_; }
  uint64_t getSumCombinedEpollUs() const override;
...
private:
  ...
  std::atomic<uint64_t> sum_combined_epoll_us_{0};
